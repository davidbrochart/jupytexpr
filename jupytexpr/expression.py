...
class Expression():
    ...
    def evaluate(self, values={}, to_cluster=False):
        if to_cluster:
            tasks = []
            var = self.variables()
        ...
        for item in self.tokens:
            ...
            if type_ == TNUMBER:
                ...
                if to_cluster:
                    ...
                    nstack.append(f(n1, n2, cluster=False))
            elif type_ == TVAR:
                ...
                if to_cluster:
                    ...
                    nstack.append(f(n1, cluster=False))
            elif type_ == TFUNCALL:
                n1 = nstack.pop()
                f = nstack.pop()
                if callable(f):
...
class Parser:
    ...
    def sin(self, a, c=None, cluster=True):
        if cluster:
            return ('unary_func', 'sin', a, c)
        else:
            return np.sin(a)

    def cos(self, a, c=None, cluster=True):
        if cluster:
            return ('unary_func', 'cos', a, c)
        else:
            return np.cos(a)

    def add(self, a, b, c=None, cluster=True):
        if cluster:
            return ('binary_func', 'add', a, b, c)
        else:
            return np.add(a, b)

    def sub(self, a, b, c=None, cluster=True):
        if cluster:
            return ('binary_func', 'subtract', a, b, c)
        else:
            return np.subtract(a, b)

    def mul(self, a, b, c=None, cluster=True):
        if cluster:
            return ('binary_func', 'multiply', a, b, c)
        else:
